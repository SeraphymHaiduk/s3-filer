...
FilerS3ClientWrapper::FilerS3ClientWrapper() 
: m_awsClientConfig(createAwsClientConfig()),
  m_awsCredentials(Aws::Auth::AWSCredentials("admin", "adminadmin")),
  m_s3Client(m_awsCredentials, 
             m_awsClientConfig,
             Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy::Never, false)
{}

Aws::Client::ClientConfiguration FilerS3ClientWrapper::createAwsClientConfig()
{
...
FilerS3ClientWrapper& FilerS3ClientWrapper::instance() 
{
    static FilerS3ClientWrapper client;
    return client;
}

bool FilerS3ClientWrapper::isConnectionEstablished() {
...
const Aws::S3::S3Client& FilerS3ClientWrapper::getClient()
{
    return m_s3Client;
}

int FilerS3ClientWrapper::createFile(const std::string& bucket, const std::string& filePath) {
    std::string emptyBody;
    return uploadFile(bucket, ensureNoStartSlash(ensureNoEndSlash(filePath)), emptyBody);
}

int FilerS3ClientWrapper::createDir(const std::string& bucket, const std::string& path) {
    std::string emptyBody;
 ...
 */
int FilerS3ClientWrapper::uploadFile(const std::string& bucket, const std::string& filePath, const std::string& content)
{
    std::cout << "FilerS3ClientWrapper::uploadFile bucket: " << bucket << " filePath: " << filePath << std::endl;
...
int FilerS3ClientWrapper::deleteFile(const std::string& bucket, const std::string& filePath)
{
...
int FilerS3ClientWrapper::deleteDir(const std::string& bucket, const std::string& dirPath)
{
    Aws::S3::Model::DeleteObjectRequest request;
    request.SetBucket(bucket);
 ...
 * This function will take care about it anyway, but use this description in case of any future changes.
 */
bool FilerS3ClientWrapper::isDirExist(const std::string& bucket, const std::string& path) {
    ...
    std::cout << "FilerS3ClientWrapper::isDirExist dir imitation file with a key: " << key << " exists: " << outcome.IsSuccess() << std::endl;
    if (outcome.IsSuccess()) {
        return true;
    }
    ...
    if (list_request_outcome.IsSuccess()) {
        ...
        std::cout << "FilerS3ClientWrapper::isDirExist outcome success. contents.empty(): "
...
Aws::S3::Model::HeadObjectOutcome FilerS3ClientWrapper::getObject(const std::string& bucket, const std::string& key)
{
    Aws::S3::Model::HeadObjectRequest request;
    Aws::S3::Model::HeadObjectOutcome outcome;
 ...
 * This function will take care about it anyway, but use this description in case of any future changes.
 */
std::optional<struct stat> FilerS3ClientWrapper::getStat(const std::string& bucket, const std::string& path)
{
    struct stat stat_data;
    
    if (path == "/" || isDirExist(bucket, path)) {
        stat_data.st_size = 0;
    ...
    std::cerr << "FilerS3ClientWrapper::getStat: no object or folder found for bucket: " << bucket << " key: " << path << std::endl;
    return std::nullopt;
}
 ...
 * This function will take care about it anyway, but use this description in case of any future changes.
 */
std::optional<std::vector<ElementWithStat>> FilerS3ClientWrapper::readdir(const std::string& bucket, const std::string& path)
{
    Aws::S3::Model::ListObjectsV2Request list_request;
    list_request.SetBucket(bucket);
    ...
    if (list_request_outcome.IsSuccess()) {
        ...
        std::cout << "FilerS3ClientWrapper::readdir outcome success. contents.empty(): "
            << (result.GetContents().empty() ? "true" : "false")
            << " commonprefixes.empty(): "
            << (result.GetCommonPrefixes().empty() ? "true" : "false")
            << std::endl;
        ...
        if (!(result.GetContents().empty() && result.GetCommonPrefixes().empty())) {
            for (const auto &prefix: result.GetCommonPrefixes()) {
                std::cout << "FilerS3ClientWrapper::readdir: " << path << " common prefix: " << prefix.GetPrefix().c_str() << std::endl;
            ...
            for (const auto &content: result.GetContents()) {
                std::cout << "FilerS3ClientWrapper::readdir: " << path << " content: " << content.GetKey().c_str() << std::endl;
            ...
            std::cout << "FilerS3ClientWrapper::readdir result paths vector length: " << paths.size() << std::endl;
            return paths;
        }
    }

    std::cerr << "FilerS3ClientWrapper::readdir error: " << list_request_outcome.GetError() << std::endl;
    return std::nullopt;
}

int FilerS3ClientWrapper::readFile(const std::string &bucket, const std::string &path, char *buf, size_t size, off_t offset) {
...
int FilerS3ClientWrapper::renameSingleObject(const std::string &bucket, const std::string &old_key, const std::string &new_key) 
{
    std::cout << "FilerS3ClientWrapper::renameSingleObject bucket: " << bucket << " old_key: " << old_key << " new_key: " << new_key << std::endl;
...
int FilerS3ClientWrapper::renameFolderRecursively(const std::string &bucket, const std::string &old_path, const std::string &new_path)
{
...
int FilerS3ClientWrapper::rename(const std::string &bucket, const std::string &path, const std::string &new_path)
{
    std::cout << "FilerS3ClientWrapper::rename bucket: " << bucket << " path: " << path << " new_path: " << new_path << std::endl;
    // single file case
    int result = renameSingleObject(bucket, ensureNoEndSlash(path), ensureNoEndSlash(new_path));
    if (result != -ENOENT) 
        return result;
    std::cout << "FilerS3ClientWrapper::rename not a file. Trying to copy as a folder" << std::endl;
