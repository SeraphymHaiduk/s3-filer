...
namespace {
    ...
    int s3filer_getattr(const char *path, struct stat *stbuf, fuse_file_info *fi)
    {
        ...
        std::optional<struct stat> result = S3ClientFacadeManager::getS3ClientFacade()->getStat(bucket_default, path);
    ...
    {
        if (S3ClientFacadeManager::getS3ClientFacade()->createDir(bucket_default, path) == 0)
            return 0;
        return -EIO;
    }
    ...
    int s3filer_unlink (const char *path)
    {
        ...
        return S3ClientFacadeManager::getS3ClientFacade()->deleteFile(bucket_default, path);
    }
    ...
    int s3Filer_rmdir(const char *path)
    {
        ...
        std::optional<std::vector<ElementWithStat>> readdir_result = S3ClientFacadeManager::getS3ClientFacade()->readdir(bucket_default, path);
        ...
        return S3ClientFacadeManager::getS3ClientFacade()->deleteDir(bucket_default, path);
    }

    int s3filer_rename (const char *path, const char *new_path, unsigned int flags) {
        return S3ClientFacadeManager::getS3ClientFacade()->rename(bucket_default, path, new_path);
    }

    int s3filer_opendir(const char *path, fuse_file_info *fi) {
        if (S3ClientFacadeManager::getS3ClientFacade()->isDirExists(bucket_default, path))
            return 0;
                ...
                enum fuse_readdir_flags flags)
    {
        ...
        std::optional<std::vector<ElementWithStat>> result = 
            S3ClientFacadeManager::getS3ClientFacade()->readdir(bucket_default, path);
        ...
        if (result.has_value()) {
            ...
            // Если S3ClientFacade::readdir возвращает временный объект, 
    ...
    int s3filer_truncate(const char *path, off_t size, fuse_file_info *fi) {
        ...
        return S3ClientFacadeManager::getS3ClientFacade()->uploadFile(bucket_default, path, "");
    }
    ...
    int s3filer_open(const char *path, fuse_file_info *fi) {
        ...
        if ((fi->flags & O_CREAT) && !result.has_value()) {
            bool creationResult = S3ClientFacadeManager::getS3ClientFacade()->createFile(bucket_default, path);
            if (creationResult != 0) 
                return -EIO;
    ...
    int s3filer_read(const char *path, char *buf, size_t size, off_t offset,
                fuse_file_info *fi) 
    {
        return S3ClientFacadeManager::getS3ClientFacade()->readFile(bucket_default, path, buf, size, offset);
    }    
    ...
    int s3filer_create(const char *path, mode_t mode, fuse_file_info *fi) {
        if (S3ClientFacadeManager::getS3ClientFacade()->createFile(bucket_default, path) != 0) {
            return -EIO;
        }
    ...
    /** 
     * TODO: Assign NULL to all unimplemented
     */
    static const struct fuse_operations s3filer_operations = {
...
extern int __test_s3filer_readdir (const char *path, void *buf, fuse_fill_dir_t filler,
                ...
                enum fuse_readdir_flags flags)
{
    return s3filer_readdir(path, buf, filler, offset, fi, flags);
}

extern int __test_s3filer_releasedir (const char *path, fuse_file_info *fi)
{
    return s3filer_releasedir(path, fi);
}

extern int __test_s3filer_chmod(const char *path, mode_t mode, fuse_file_info *fi)
{
    return s3filer_chmod(path, mode, fi);
}

extern int __test_s3filer_chown(const char *path, uid_t uid, gid_t gid, fuse_file_info *fi)
{
    return s3filer_chown(path, uid, gid, fi);
}

extern int __test_s3filer_truncate(const char *path, off_t size, fuse_file_info *fi)
{
    return s3filer_truncate(path, size, fi);
}
